...
pub async fn index() -> Result<HttpResponse, CustomError> {
    ...
    let mut file = match File::open(&index_path) {
        Ok(file) => file,
        Err(_why) => return Err(CustomError::StaticNotFound),
    };

    let mut index_content = String::new();
    match file.read_to_string(&mut index_content) {
        Ok(_) => Ok(HttpResponse::Ok().body(index_content)),
        Err(_why) => Err(CustomError::StaticInternalError),
    }
}

//the static files has been built
#[get("/{filename:.*\\..*}")]
pub async fn static_file(req: HttpRequest) -> Result<HttpResponse, CustomError> {
